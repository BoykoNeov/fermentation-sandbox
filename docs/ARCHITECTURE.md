...
**What it is not:** a history. *Why* anything is shaped this way and what was tried and rejected
live in [`DECISIONS.md`](DECISIONS.md), which is the canonical archive; *what is still open* is
[`OPEN.md`](OPEN.md), generated by `tools/gen_open_ledger.py` from the suite's `xfail` markers and
the archive's open `Flags:` pairs — derived, never written.
...
`uv run pytest -n auto` (95 test files; unit, integration, conservation, provenance,
...
step, which is what made the failures easy to misread). CI also runs the two doc generators in
`--check` mode. Three of the test files guard documentation rather than physics:
`test_decisions_index.py` (the archive's generated index), `test_open_ledger.py` (the generated
open ledger) and `test_memory_shape_hook.py`.

A beat that measures rather than builds keeps its probes, its pre-registration and its findings
under `docs/receipts/<d-number>-<slug>/` when it ran somewhere without the owner's receipts drive
(first: `d266-joint-fusel-repair/`). Those folders are frozen artefacts beside the JSON they
produced and are excluded from lint; the guards that pin their findings live in `tests/`.
