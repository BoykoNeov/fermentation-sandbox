...
- [Project: Fermentation Sandbox](project-fermentation-sandbox.md) — status + repo; M0/M1/M2 complete, aging build order fully built, Milestone-3 at D-164 (D-163 swept band edges archive-wide — 73 arms, **19 of 678** guarded, **14 of 18 files wholly unguarded** — and found a channel in which an *epistemic* band silently gates a scenario override; D-164 then CLOSED that channel at the seam, pinned 4 edges, and **rejected the structurally-clean decoupling candidate on measurement**, leaving a bigger **wide-band ensemble** artefact open). Prohibitions + pointers only; per-decision archive is docs/DECISIONS.md
