...
- [Project: Fermentation Sandbox](project-fermentation-sandbox.md) — status + repo; M0/M1/M2 complete, aging build order fully built, Milestone-3 at D-162 (D-161 restated the pKa-affected bands — one archive row, seed labels **sorted**, the fix **undetectable through the shipped sampler**; D-162 then SCOPED the closure ordering into **three** claims of three strengths, found ~9/10 of the fragile one's inversion is band **scope** not measurement overlap, and pinned the band **edges** that nothing had guarded). Prohibitions + pointers only; per-decision archive is docs/DECISIONS.md
