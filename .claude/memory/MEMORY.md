...
- [Project: Fermentation Sandbox](project-fermentation-sandbox.md) — status + repo; M0/M1/M2 complete, aging build order fully built, Milestone-3 at D-161 (D-160 declared the 13 runtime-read pKa/SO2-binding names, closing D-159s live sampler gap; D-161 then RESTATED the affected bands and found the class is **one archive row**, printed with its three seed labels **sorted**, and that the fix is **undetectable through the shipped sampler** with the predicted widening **not observed**). Prohibitions + pointers only; per-decision archive is docs/DECISIONS.md
