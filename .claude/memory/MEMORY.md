...
- [Project: Fermentation Sandbox](project-fermentation-sandbox.md) — status + repo; M0/M1/M2 complete, aging build order fully built, Milestone-3 at D-163 (D-162 SCOPED the closure ordering into **three** claims of three strengths and pinned the band **edges** nothing had guarded; D-163 then generalised that mutation to the whole archive — 73 arms, **19 of 678** band edges guarded, **14 of 18 files wholly unguarded**, and a **second channel** found in which an *epistemic* band silently gates a scenario override). Prohibitions + pointers only; per-decision archive is docs/DECISIONS.md
