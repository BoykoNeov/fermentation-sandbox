...
## Status (2026-08-27)
M0/M1/M2 **complete**. **M3** (sensory/OAV + Tier-3 aging, D-66) at **D-247** (D-231/D-242 are tooling, not model); sensory 1a/1b + **D-139's leftovers ALL closed** (D-148/9). Suite **1993 + 10 xfail** (3 D-188 Herzan; 2 D-215's own; **5 from D-244, ATTRIBUTED at D-245** — two of its six were a harness bug and a consumed margin; splitting a LOOPING guard per alcohol then made one hidden leg two named ones, see the ledger row) — MEASURED off a green full run, never inferred: the ledger read 1861 for two beats.
...
- **CI gates & solver-output equality (D-242)** — CI runs FOUR gates and `ruff format --check` is SEPARATE from
  ...
  → `.claude/memory/prohibitions/seed-reads-repair.md`
- **Wine's nitrogen budget (D-243) → PARTITIONED (D-244) → ATTRIBUTED (D-245)** — audit CLOSED, its open item
  SPENT: `yan_mgl` is the must's **TOTAL** assimilable N, the pools are CARVED OUT, and the fit is evaluated
...
**D-246 ANSWERED that ask and it CLOSED NOTHING — do not re-propose it.** Both papers use the same Bely 1990 medium; Crepin's Data Set S1 holds it MEASURED (mM, all 20 species, mean of 14) and Minebois prints the stock verbatim. Scored on their OWN musts: propanol 0.7963 vs the 0.80 floor (the must was 86 % of the gap, not all of it), and BOTH Minebois legs get **worse** — ~1.7x her measurement where the 'unfairly rich' fixture read ~1.1x. Pinned in `tests/test_defined_media.py`; the xfail reason strings no longer say 'we lack the medium'. **What it bought instead, and the real open item: the model leaves 40.8 % of Crepin's assimilable nitrogen standing where she measures 0.2 %** — not slowness (identical at 30 and 60 days), but growth-coupled uptake consuming only what growth demands. THREE CORE REPAIRS were named; **one is CLOSED — measured and REFUSED at D-247 — and two remain the owner's call**: (i) `yan_mgl` carries two meanings (total N vs assimilable N — 201.11 vs 179.91 on one must); (ii) un-couple N uptake from growth demand (the largest, and the one with a published number to score against); ~~(iii) `depletion_gate`'s must-SPECTRUM scaling~~ — **CLOSED at D-247: MEASURED, REFUSED, and it CORRECTS D-246 §6.** The composition-only correction (each share re-referenced to the declared must, spectrum sum preserved) moves propanol **0.796275 → 0.796017** — 6.9 % of the gap the WRONG way, stable across four decades of solver tolerance, ≤0.5 % on every fusel with all Processes live. D-246's probe cleared the floor only because scaling `K_amino_acids` uniformly is a **LEVEL** change, bit-identical to scaling all eight shares. **The residual miss is UNATTRIBUTED, not gate-attributed.** The reference is unsourced either way (D-100 declined per-species Km as the D-98 trap), so **only literature half-saturations settle it — a sourcing ask, never a core change**. Do not re-propose the repair; its compile-seam design is priced in D-247 §5 (mint gate-scale params, NEVER overwrite the shares — `spectrum_carbon_per_nitrogen` reads them and D-34's debris-carbon sign leans on that ratio). Migrating the guards themselves onto the papers' musts is a separate owner's call (two would fail harder). — `prohibitions/aroma-and-milestone-3-tail.md`.
