...
metadata:
  ...
  modified: 2026-08-31T15:57:02.994Z
---
...
## Status (2026-09-06)
M0/M1/M2 **complete**. **M3** (sensory/OAV + Tier-3 aging, D-66) at **D-275** (**D-275 is a LEDGER-MACHINERY beat, no `src/` and no model test: one of the SEVEN rows in `docs/OPEN.md` printed a Python VARIABLE NAME as its reason and linked NO record -- `reason=` was resolved against the marked file alone and that constant is imported from a sibling test module. Now resolved across modules to a FIXED POINT (re-export, relative, `as`, local shadowing); the row links six records. **The file's COUNTS were never wrong and `--check` never went red -- an unreadable row is not a missing row, and nothing was counting readability**; a new bare-identifier count lands INERT at 0. Capping the now-1.9k-char cell was REJECTED: the constant leads with superseded numbers and TRAILS with its own `[HISTORY, NOT CURRENT STATE]` notice, so truncation prints retracted measurements as current. Its own title then matched NO `TOPIC_RULES` bucket -- needles name the SURFACE (`open ledger`/`what is open`/`open.md`), never bare `ledger`, which six nitrogen/colour titles carry. Suite **2189 + 7 xfail MEASURED** off a green BelowNormal `-n auto` run, 1794 s -- the +8 over D-273's 2181 is **5 from D-274, a beat that recorded NO count**, plus D-275's 3; two earlier runs read 2184/2188 because they overlapped live edits and ANOTHER session's suite on this box. -> `lessons/records.md`**) (**D-274 is a MEMORY-HYGIENE beat, no `src/` and no model test: D-273's correction reached `beer-acid-course-timing.md`'s description and body but NOT its header, and — the copy that mattered — NOT the LEDGER row here, which still read "the three timings OPPOSE and nearly cancel" on the one surface loaded every session. **The CAP was NOT the obstacle** (the header block is 6 of 8; the fix adds zero lines; what sat at 8/8 was the ledger row, a different block in a different file) — **nothing was raised and nothing evicted**. The standing "go read D-N" line is BOILERPLATE and 16 of 33 files hard-coded a record in it; all 16 now say "its D-record" and `check_memory_size.py` forbids the number, landing INERT. A general staleness linter off the correction map was measured (**547** findings whole-file, **155** header-only) and REJECTED: the map's edges are per-CLAUSE, so record granularity cannot say "corrected on the clause you quote". → `lessons/records.md`**) (**D-273 CORRECTS D-215 §3 — its "the three beer acid timings OPPOSE / no single correction helps all three" was the FRAME, not the acids: it scored them against the model's OWN parked ~3× day-2 speed deficit. In Tyrell's own frame all three TRAIL the sugar, succinic < lactic < malic (+13.5/+44.8/+63.5 pts at day 2), and the deficit lands INSIDE that span, which is what flips the signs. **Never read a per-acid SIGN off model-minus-source here**, and succinic is the SMALLEST defendant, not a control. The frame-free gap is NEW and is the 7th xfail: his wort is out at day 5, lactic still gains 21.6 % of its rise, a `Y·flux` producer can place 0.989 % — a SOURCE is missing, not a rate, so no speed fix reaches it. NOTHING BUILT and priced: 5× lag spread needs 3 fitted magnitudes, no clock to score a timing repair on, autolysis reported with OPPOSITE signs by 2 of the 5 beer texts. Suite **2181 + 7 xfail MEASURED** off a green BelowNormal `-n auto` run, 752 s — the +3 are D-273's passing guards and the xfail SET moves 6 → 7, the first increase in many beats. → `lessons/measurement.md`'s residual row**) (**D-272 is the OWNER'S ADJUDICATION of D-270's priced trade: beer's sourced cell-nitrogen composition is DECLINED and 0.114 stays -- NO `src/` change, NO new guard, NO marker, and D-270's flag on D-14 deliberately LEFT OPEN because a refusal is not a shipped fix, so an OPEN row there means 'declined at a price', not 'awaiting a build'. Suite **2178 + 6 xfail MEASURED** off a green BelowNormal `-n auto` run, 640 s on a shared box -- UNCHANGED, as a no-guard beat must be. -> `prohibitions/beer-growth-extent.md`**) (**D-271 is a SOURCING HUNT that came back EMPTY -- 11 new guards, no model change; suite **2178 + 6 xfail MEASURED** off a green BelowNormal `-n auto` run, 1308 s on a shared box -- the +11 are D-271's guards and the xfail SET is unchanged**) (**D-269 REFUSES the owner-gated joint fusel repair — a DECISION, no code; D-270 is a FRAME correction + census, `src/` prose only. Suite 2167 + 6 xfail MEASURED off a green BelowNormal `-n auto` run, 591 s; D-270's 10 new guards are the +10 and the xfail SET is unchanged**) (D-231/D-242/**D-261**→**D-265** are tooling+interface, not model; **D-266/D-267/D-268 are MEASUREMENT+SOURCING beats -- NONE touches the model; suite 2157 + 6 xfail MEASURED off a green `--group ui -n auto` BelowNormal run (1173 s on a SHARED box), D-268's one new guard being the +1 and the xfail SET unchanged** — the 2131 below is D-265's); sensory 1a/1b + **D-139's leftovers ALL closed** (D-148/9). Suite **2131 + 6 xfail** — MEASURED off a green `-n auto` run, 397 s (and **on CI at D-265: 2131 + 6 on BOTH 3.13 and 3.14, the first time any test ran on 3.13**) (**D-261→D-264 add 48 app guards between them and NO model test**; the 2083 body is unchanged; **D-264's are the launcher, the report folder and the absolute-path scan**) (D-257→D-260 add 1, 2, 4 and 5; only D-260 touches production code, and only a DOCSTRING; the xfail SET is unchanged) (3 D-188 Herzan; 2 D-215's own; **1 fusel** — D-244's five became one at **D-248**, which closed four by un-coupling nitrogen uptake and lowered NO sourced threshold) — MEASURED off a green full run, never inferred: the ledger read 1861 for two beats. Slot/Process/oxidative counts live in `docs/ARCHITECTURE.md` — never restate here, that rotted it (D-184).
...
**D-213 BUILT beer's wort O₂ — sourced and INERT; the O₂→growth coupling was RE-OPENED by the owner at D-258, worked, and REFUSED on measurement — do not re-propose it, least of all on "the predictions are reachable now" (that IS D-258 §3).** The target contradicts itself: a ceiling landing Tyrell's counted fold breaks Tyrell's counted TIMING, same panel. The beat's real find is a SECOND, PRINTED extent — "the yeast will multiply four- or fivefold" — which sat between two sentences D-213 already quoted, and re-scales the audit's headline 1.546x to **1.076x**. **Never quote 1.546x as the model's extent error unqualified.** → `prohibitions/beer-growth-extent.md` + `prohibitions/beer-wort-oxygen.md`.
...
- **The CI harness itself (D-265)** — CI was red for **eleven straight pushes** (D-261→D-264) and is
  green now. Every `uv` step in `.github/workflows/ci.yml` carries **`--group ui` AND `--python`** —
  **five pairs, all load-bearing, NEVER delete them as boilerplate.** Without the group, mypy checks
  `app/` with streamlit absent, `st.stop()` loses `NoReturn` and 44 phantom errors land on correct
  code; without the pin, a bare `uv run` takes the newest interpreter, **deletes the venv and
  rebuilds it** — which is why the "3.13" job had been running 3.14 since the matrix existed. The
  app's 48 guards `importorskip`, so a groupless CI goes green having tested **none** of the console.
  → [[feedback-a-green-matrix-may-vary-nothing]], [[feedback-green-here-red-there-is-the-environment]]
- **The console / `app/` (D-261 → D-264)** — a Streamlit app + written HTML report **IS BUILT** and is a
  ...
  **D-262/D-263 answered the owner's four reports — do not re-propose any of them as missing.**
  Both grounds ship (`[theme.light]`/`[theme.dark]`; `TIER_STYLE` is NOT per-theme, on purpose);
  a log scale is a sidebar toggle, floored six decades under each axis's own peak and CLAMPING
  rather than dropping; an all-inert chart explains itself in words; and the number boxes sit in
  a form so a stepper no longer re-runs the model per click. Two live constraints: switching the
  PAGE theme does not re-run the script, so charts follow at the next click (the server cannot
...
- **Growth-anchored precursor sink (D-259 → D-260)** — "exactly reversed" CORRECTED (ile > val
  ...
  ANSWERED, not open — the split and Rollero's tracer are ONE knob** (pool fully consumed, isoamyl
  inert), shipped at the Crépin end, growth-anchored at the Rollero end, neither the other's repair.
  D-259's bracket was the PRE-MODIFIER frame → **21.3-33.7 %**, so D-104's 20.9 % sits BELOW it, not
  at the top. Live question is the isoamyl DENOMINATOR (commensurability, **not** re-fitting
  `k_isoamyl_alcohol`). **Build stays owner-gated.** → `prohibitions/aroma-and-milestone-3-tail.md`
- **The JOINT repair, blend + growth sink (D-266)** — RUN for the first time; neither refusal had measured
  the pair. It clears D-257's blocker and keeps the blend's timing; the split rises 27.6 → 54.5 % with the
  tracer inside Rollero's band -- **RETRACTED at D-268, next block: on the SOURCED composition the tracer is
  BELOW the floor at every edge and D-266's own pre-registration was right**; isoamyl on Crépin's must
...
- **The re-pricing onto the sourced composition (D-268)** — D-267's flag RETIRED. `w_i` is now **DERIVED
  from the Table IV transcription** (no second literal typed); the residue half is a measurement, only the
  protein fraction is still a bracket. Splits rise 6-11 points; brackets HALVE in width. **D-266's headline
  is RETRACTED — the joint arm's leucine tracer reads 3.13 % (2.93-3.36), BELOW Rollero's 3.4 % floor at
  EVERY edge. The owner's fork now has ONE direction, not two.** D-260's "λ=5 lands Crépin's band" was FALSE
  on its own composition (73.3 vs her 77 floor; guard pinned at 70) and is TRUE on the sourced one (77.8).
  **Superseded arms kept LIVE** so D-260's/D-266's published numbers stay reproducible — never delete them.
  → `docs/receipts/d268-*`, [[feedback-a-gap-needs-both-sides-from-the-same-run]]
- **The count+weight pairing hunt (D-271)** -- **EMPTY, and do not re-run it.** D-219's open item
  (a cell COUNT paired with a gravimetric DRY WEIGHT, one ferment, one timepoint) is not published:
  four sources miss it four ways because **each reports the ONE currency its endpoint needs**, and
  2 of the 4 were already at D-219 §4. Chemostat work does not count cells. **Every per-cell mass in
  reach lands BELOW branch 1's 70.9-91.9 pg** -- a DIRECTION, not a settlement. **The size route is
  FRAME-BROKEN (4.63x/5.24x on the same strains) -- never price branch 1 by cell size.** D-219's
  cross-check is re-SOURCED (37.7-56.6 pg), its 28-50 band UNTOUCHED; 47.71-62.12 is the ENGINE's
  estimate, never the demand. → `prohibitions/beer-growth-extent.md`
...
- **Beer pH: the LITERATURE has none; the acid COURSES were MIS-FRAMED (D-215, its §3 CORRECTED at
  D-273)** — **NO model change in either.** Corpus null on a missing acidifier: Ca-phosphate is **BOIL**
  (pre-pitch, like trub), H⁺-ATPase is **LAB/wine only**, no beer BC-during-ferment, **Tyrell is still
  the ONLY pH curve**. **D-215 §3's "the three `Y·ΔS` timings OPPOSE and nearly cancel" is RETRACTED** —
  that was the model's own parked speed deficit crossing three lags; in Tyrell's frame **all three TRAIL**,
  and a per-acid SIGN must never be read off model-minus-source. One knob still cannot fix the shape (lags
  span ~5×). **D-273's 7th xfail: the acids rise on after the wort is OUT — a SOURCE is missing, not a
  rate.** **~3.2× slow at day 2**, but **NOT** the day-1 pH cause; pH probe **INCONCLUSIVE**. → `.claude/memory/prohibitions/beer-acid-course-timing.md`
...
- **D-219 — a cell's dry mass is a UNIT DEFINITION, not a literature question; §2.2's window FAILS.**
  **4e-11 g/cell**: Coleman 2007 (wine's `Y_X/N`/`k'_d`/`mu_max` source) says *"assuming that each cell
  ...
  6.58× day-2; pH 7/8→6/8). **2.51× and D-215's day-2 slowness COMPOUND, not corroborate** (its
  "~2.8×" is **~3.2×** since D-273 re-measured it off D-222's pitch and D-223's rate).
  [[feedback-a-constant-can-be-a-unit-definition]] → `.claude/memory/prohibitions/yeast-cell-dry-mass.md`

- **Cell nitrogen & the joint fusel repair (D-269, D-270)** — **BOTH CLOSED AS DECISIONS; nothing
  built.** **D-269 REFUSES** the build D-266 §9 reserved: on D-268's sourced composition it misses
  BOTH leucine observables at every bracket edge (split 57.8-63.1 vs 77-86; tracer 2.93-3.36 vs a
  3.4 floor), over-shoots the three imposed splits 3-8 points further, and improves only a ratio
  whose labelled amount is **1.35-2.10x** the source — **two errors cancelling is not an axis.**
  **D-270 CORRECTS D-267 §6's FRAME**: `biomass_N_fraction` is a **yield residual** in wine
  (0.0362-0.1068), not a composition; the mismatch is beer's band and is **flagged, not repaired**
  (extent 5.378x → 7.4-9.6x). → `prohibitions/yeast-cell-dry-mass.md`
...
- **The per-member pH ANCHOR + the peptide pair (D-233)** — the anchor re-solve is **BUILT**, BOTH
  ...
  **1.008×**) — a PER-MEMBER error, argued from **t=0 only**. **Capacity half is BUILT at D-246**
...
- **CI gates & solver-output equality (D-242)** — CI runs FOUR gates and `ruff format --check` is SEPARATE from
  ...
  → `.claude/memory/prohibitions/seed-reads-repair.md`
- **Wine's nitrogen budget (D-243) → PARTITIONED (D-244) → ATTRIBUTED (D-245) → UN-COUPLED (D-248)** — audit
  CLOSED: `yan_mgl` is the must's **TOTAL** assimilable N, pools CARVED OUT, fit **HELD** at Coleman's 350 edge
  (EPISTEMIC, never saturation; low edge unenforced — a low hold moves Varela's 50 arm). **Summing DOES NOT RUN.**
  **D-248 BUILT the big repair**: uptake reads `mu_max` as a CAPACITY, never the growth rate, so the must is
  consumed (40.8 % → 0.62 % standing) and biomass reaches **98.4 %** of the Coleman yield the seam installs
  (was 61.6 %). **FOUR fusel xfails closed, ZERO thresholds lowered**; one remains. **`yan_mgl`'s two meanings
  are MEASURED and REFUSED** — the frames cancel exactly; the seam-only fix costs 6.1 %. **Never lower the 80 % floor.**
  → `prohibitions/wine-nitrogen-budget.md` + `prohibitions/aroma-and-milestone-3-tail.md`
- **The uptake surplus is INTRACELLULAR (D-250, corrects D-248 twice)** — D-248 refunded it to `N`, which the
  charge balance READS, so nitrogen inside the cell titrated the must: **+0.215 pH** mid-run on a 2 g/L dosed wine,
  `N` 300→436.8. **Conserved to 1e-8 throughout — the defect is CHARGE, not mass**; never restate it as creation.
  `stored_nitrogen` is in **no** charge balance; growth's Monod + draw and the swap's refund use **ONE proportional
  split helper** (refund wholly to `N` ⇒ net `dN/dt` POSITIVE and it returns). Footprint is pH ALONE: beer
  **bit-identical**, wine ≤1.9e-7 (schema, falsified on the undosed arm). **The bacterial residue is CLOSED as
  NOT-a-defect** — the store is inside a yeast cell, Brett's 96 % loss is the model being RIGHT, the real gap is
  **peptides**, and the repair UN-MASKED the malate sign (it FLIPPED across the dose; now 4.66x/1.18x, both pinned).
  Never teach a bacterium to read the store. → `prohibitions/nitrogen-storage-and-charge.md`
- **BOTH nitrogen owner's calls are TAKEN (D-253, corrects D-251/D-248)** — fixture pitch **0.04** (Minebois'
  1e6 cells/mL, sourced for HER, lineage for Crepin) and `amino_acid_uptake_capacity_ratio` **2.6**. **ORDER is
  load-bearing** — pitch first, the landing `r` is NOT pitch-invariant. Buys the clock (29.9 h vs her 28; 28.6 h
  in her own frame); costs peak biomass 3.21 vs 3.39. **The EXTENT never moved** (0.623 % at every r 0.25–50) so
  the level is landed on the SHAPE, defended by the 3/4 landmark + RATIO arriving uninvited, **not by the tier**.
  **±0.2 precision — never a third digit.** Minebois does NOT regress (the half D-252 never priced). Three guards
  were reading something else: a multiplier-keyed sweep, a `== 0.0` reading solver-noise SIGN, and D-251's 1e-9
  brittleness, which FIRED. → `prohibitions/wine-nitrogen-budget.md`
- **The fusel-guard migration is CLOSED — measured, and the migration REFUSED (D-254, corrects D-244/D-111)** —
  D-246 §7's open ask. Its stated reason had EXPIRED: two fixtures' verbatim comment said the papers' medium is
  "not in this repo" eight records after D-246 put it there — **8th stale-sourcing instance, and the first living
  in CODE**. Rollero's twin comment is STILL LIVE (checked) but narrowed to one question: is his SM Bely 1990?
  What blocks migration now is a MODEL fact: `_amino_acid_share` credits DESIGNED shares while `ehrlich_draws`
  truncates the valine→isoamyl branch, and the total is pinned at `1-f`, so the bias TRANSFERS. On Minebois's own
  must one alcohol over-attributes EITHER WAY and the two estimators name DIFFERENT ones (harness isoamyl 1.014x,
  realised isobutanol 1.049x). **Never relax that guard to migrate it.** → `tests/test_fusel_provenance_estimator.py`
- **Rollero's medium is SOURCED, the D-115 probe MIGRATED (D-255, corrects D-111/D-244)** — D-254's open question,
...
- **The isoamyl timing repair was BUILT and REVERTED (D-257, corrects D-256) — do NOT re-propose it as
  unbuilt, and do NOT re-propose it as available.** A blended fusel gate `f*N/(K_n+N) + (1-f)` at f=0.79
  ...
  **The real blocker is D-104's sink** — precursor consumption (protein, the DOMINANT fate) rides higher-alcohol
  production instead of growth; fix that first and this repair reopens. Patch + probes kept at
  `M:\claud_projects\temp\ferment\d257-fusel-de-novo\`. Also measured there: the attribution layer must scale
  by the Ehrlich share or the D-117 carbon margin goes to 1.147x, and **propanol drops to 79.1 % de novo under a
  sourced 80 % floor** with the obvious `f_de_novo_propanol` cap REFUSED under D-206 (it would be fitted to the
  model's own abundances). **The leucine tracer NUMERATOR did not move at all** (11.9/42.6/72.4 uM, bit-stable) —
  the enrichment % moved only because its denominator did, the third beat running.
...
**D-246 ANSWERED that sourcing ask and it CLOSED NOTHING — do not re-propose it.** Both papers use Bely 1990; Crepin's Data Set S1 holds it MEASURED and Minebois prints the stock. What it bought was the real finding: **the model left 40.8 % of Crepin's assimilable N standing where she measures 0.2 %**, and it named THREE core repairs. **ALL THREE ARE NOW RESOLVED — none is open.** (iii) `depletion_gate`'s must-SPECTRUM scaling: MEASURED and **REFUSED** at D-247 (it moves propanol the WRONG way; D-246's probe was a LEVEL change), and D-248 makes both rescalings **inert**. (ii) un-couple N uptake from growth demand: **BUILT at D-248** — the must is consumed, biomass reaches 98.4 % of the Coleman yield the seam itself installs, and **four of the five fusel xfails close with no sourced threshold lowered** (propanol 0.7963 → 0.8784 on Crepin's must; both Minebois legs 1.7x → ~1.0x her published shares). (i) `yan_mgl`'s two meanings: **MEASURED and REFUSED at D-248** — the carve-out and release frames cancel *exactly*, so the shipped seam is outcome-correct, and repairing the declaration alone costs **6.1 %** of it; the complete version (arginine 3-of-4, the fourth parked as excreted urea) is PRICED, not built. **The exhaustion TIMING is CLOSED at D-249 — attributed and REFUSED, do not re-propose it.** Read against the fermentation containing it the nitrogen is SLOWER than its own run (1.59x vs the run's 1.92x; at the sourced pitch 0.94x vs 1.54x — 0.94 means slightly SLOW, not fast, unlike every other ratio in this line) and time-free it exhausts LATER in the run than Crepin (0.225/0.308 vs her 0.187); the whole-run gap is the already-guarded Coleman-vs-Varela clock (D-56/D-57). Swept to **1000x** the capacity SATURATES at 16.4 h and never overtakes the run, so the knob is **unreachable, not deferred**. D-249's time-free shape defect is **NARROWED at D-251 and its FORM verdict WITHDRAWN — never re-propose a replacement rate law as a settled need.** 0.543 was read in the MODEL's frame; Crepin sampled the **MEDIUM** and the model's quantity counts the store too — in her frame it is 0.412, so two fifths of the gap was the frame. The **shipped** capacity knob then crosses her 0.245 at r~3.9 (2.65 at the sourced pitch), INSIDE its declared [0.5, 10] band; D-249's sweep missed it because the total frame is BLIND to that knob (0.543→0.542 across 1000x). Her THIRD landmark discriminates nothing. **TAKEN AT D-253 — see the ledger row above; through D-252 it was NOT taken and was priced the way D-251 priced it**: the cell-nitrogen objection is VOID (an end-state band used as a ceiling on a transient peak at a moment Crepin never sampled; at her landmark the calibration UNDER-loads), and D-249's "moving the pitch sells the Coleman anchor" is VOID too (the guard hardcoded the fixture's pitch). What remains: the landing r is not pitch-invariant, the full-suite cost at r=3.9 is 2 re-pins / 2041 pass, and **Crepin's data CANNOT settle the inoculum** — separating 0.04 from 0.25 needs `biomass_N_fraction` to 6.7 % and its declared band spans a factor 1.75. **Both moves were the owner's and BOTH WERE CALLED: D-253 pitches 0.04 and lands r at 2.6, in that order. Do not re-propose either.** D-248 §10 stays REFUSED — D-250 did NOT unblock a Droop quota (the rate still reads nothing about the store). **Was the owner's call, RE-PRICED at D-252 and TAKEN at D-253: the Crepin/Minebois fixture pitches 0.25 g/L, a house default — Crepin states NO inoculum (`grep -c` = 0) and the sibling paper's 1e6 cells/mL is 0.04, 6.25x smaller. It fixes the timing (29.9 h vs her 28) and costs peak biomass (+0.8 % → -5.2 % vs her 3.39). It does **NOT** break D-248's Coleman guard — that 0.984 → 0.925 was the guard's HARDCODED 0.25; against the run's own pitch it reads 0.9848. A trade, re-priced, and TAKEN at D-253 — the timing lands, Minebois (the half nobody had priced) does not regress, and the re-measured landing capacity is 2.6, not D-251's two-point 2.65.** **The bacterial-nitrogen residue (`Flags: D-100`) is CLOSED at D-250 as NOT-a-defect — do not re-propose it.** The slot uptake fills is INTRACELLULAR (`stored_nitrogen`, in no charge balance), so MLF/Brett blindness to it is correct; Brett's 96 % growth loss is the model being RIGHT, and the real gap is **peptides**, a bacterial N source yeast do not take. D-250 also found and fixed what sat under it: D-248's booking put that nitrogen in `N`, which the charge balance reads, worth **+0.215 pH** mid-run on a dosed must. **Migrating the guards onto the papers' musts is CLOSED at D-254 — measured and REFUSED, do not re-propose it.** Its stated blocker (the papers' medium 'not in this repo') had been dead since D-246; what blocks it now is that on Minebois's own must one valine alcohol over-attributes whichever estimator is read, and the two name different alcohols. **D-255 does NOT reopen that** — it sources ROLLERO's medium (the third paper, whose comment D-254 checked and correctly left live) and migrates the D-115 *enrichment* probe, a different guard. The de-novo-cap tripwire stays where D-254 put it. — `prohibitions/wine-nitrogen-budget.md` + `prohibitions/aroma-and-milestone-3-tail.md`.
