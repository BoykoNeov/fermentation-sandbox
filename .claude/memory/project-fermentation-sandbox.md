...
**D-246 ANSWERED that ask and it CLOSED NOTHING — do not re-propose it.** Both papers use the same Bely 1990 medium; Crepin's Data Set S1 holds it MEASURED (mM, all 20 species, mean of 14) and Minebois prints the stock verbatim. Scored on their OWN musts: propanol 0.7963 vs the 0.80 floor (the must was 86 % of the gap, not all of it), and BOTH Minebois legs get **worse** — ~1.7x her measurement where the 'unfairly rich' fixture read ~1.1x. Pinned in `tests/test_defined_media.py`; the xfail reason strings no longer say 'we lack the medium'. **What it bought instead, and the real open item: the model leaves 40.8 % of Crepin's assimilable nitrogen standing where she measures 0.2 %** — not slowness (identical at 30 and 60 days), but growth-coupled uptake consuming only what growth demands. THREE CORE REPAIRS are now named and all are the owner's call: (i) `yan_mgl` carries two meanings (total N vs assimilable N — 201.11 vs 179.91 on one must); (ii) un-couple N uptake from growth demand (the largest, and the one with a published number to score against); (iii) `depletion_gate` scales its half-saturation by the must-SPECTRUM shares, which the per-species override bypasses — rescaling it to the pool it actually gates carries propanol over the floor, so it spans the whole residual miss. Migrating the guards themselves onto the papers' musts is a separate owner's call (two would fail harder). — `prohibitions/aroma-and-milestone-3-tail.md`.
