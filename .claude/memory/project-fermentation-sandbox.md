...
## Status (2026-09-01)
M0/M1/M2 **complete**. **M3** (sensory/OAV + Tier-3 aging, D-66) at **D-263** (D-231/D-242/**D-261**/**D-262**/**D-263** are tooling+interface, not model); sensory 1a/1b + **D-139's leftovers ALL closed** (D-148/9). Suite **2123 + 6 xfail** — MEASURED off a green `-n auto` run, 512 s (**D-261/D-262/D-263 add 41 app guards between them and NO model test**; the 2083 body is unchanged) (D-257→D-260 add 1, 2, 4 and 5; only D-260 touches production code, and only a DOCSTRING; the xfail SET is unchanged) (3 D-188 Herzan; 2 D-215's own; **1 fusel** — D-244's five became one at **D-248**, which closed four by un-coupling nitrogen uptake and lowered NO sourced threshold) — MEASURED off a green full run, never inferred: the ledger read 1861 for two beats. Slot/Process/oxidative counts live in `docs/ARCHITECTURE.md` — never restate here, that rotted it (D-184).
