...
**NEW at D-245, owner's call — ONE ask gates FOUR of the five fusel xfails:** the **defined-medium compositions of Crépin 2017 and Minebois 2025**. Propanol's floor miss, the band restating it, and both Minebois legs are all scored on a must carrying 1.0 g/L amino acids against papers that used their own media; nothing in this repo can compose one without the D-244 §6 category error. A sourced `f_de_novo_isoamyl` is the separate, smaller ask (D-120's refusal has lost BOTH legs, but the model-abundance derivation is refused at D-206). — `prohibitions/aroma-and-milestone-3-tail.md`.
