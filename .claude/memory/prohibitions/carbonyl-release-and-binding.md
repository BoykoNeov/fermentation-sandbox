...
metadata: 
  ...
  modified: 2026-08-12T06:35:27.742Z
---
...
- **The published two-sided literal is an IMPLEMENTATION check, NOT corroboration** (amendment).
  UWC Table 17.2 prints acetaldehyde Kd **1.5e-6 = the shipped value**, and Ch. 24's ">99 % / >95 %"
  is Ch. 17's arithmetic on it — **never cite it as a different book agreeing.** What it pins is
  the **basis**: bisulfite-with-beta vs the textbook's per-total-free apparent Kd, agreeing to
  <=5 %. **The BAND does not carry it**: the weak edge gives **93.3-93.6 %**, missing the 95 %
  floor; the nominal clears by ~0.2 pp. **Never "fix" either side** — both are sourced.
...
- Shipped makes **alpha-KG the stronger** binder (1.4e-4 vs 5.55e-4). **All three locally-held
  ...
  (2.0e-4 / 6.6e-4), UWC T17.2 (1.4e-4 / 4.9e-4). D-130's footnote named two corroborators and
  **only ONE was checked**: Handbook §8.4 is **FALSIFIED** (read directly); **Barbe 2000 JAFC is
  UNVERIFIED — never cite it either way**, its disagreement was inferred from a figure caption.
