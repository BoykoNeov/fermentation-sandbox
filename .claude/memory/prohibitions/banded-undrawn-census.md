...
description: "D-240 — the banded AND compile-read AND never-drawn census: 28 names, six classes, priced and PINNED"
metadata:
  node_type: memory
  type: project
---
...
- **`o2_wort_aeration_beer` is banded 1.45×, live, never drawn, and worth EXACTLY ZERO** (final
  `X` moves 7.7e-09, `E` 5.1e-11). D-213's aging-gate scope decision, re-measured on the
  trajectory. **Never file it as a gap** — it is the zero the other prices are read against.
...
- **PINNED, not repaired, and the reason is mechanical.** Drawing one of these does nothing on
  its own: the sampler re-draws `param_values`, the seed is already in `y0`. A repair is a
  `y0_for_member` rule (D-236) **AND** a declaration — and the declaration half is a
  tier-propagation claim (`reads` has two masters, D-160) that nothing here measures. D-237
  declined exactly this. **A RED on `test_the_priced_names_are_still_undrawn` means the gap was
  CLOSED: delete the row and record the movement.**
