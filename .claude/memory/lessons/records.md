...
- [A correction reaches the copy you edited](../feedback-a-correction-reaches-the-copy-you-edited.md) — D-273's retraction landed in the file's description and body and left the HEADER and the boot-loaded LEDGER row stating it. Grep the retracted WORDS, fix the entry point first (D-274)
- [A fallback to the key hides the miss](../feedback-a-fallback-to-the-key-hides-the-miss.md) — `consts.get(name, name)` made an unresolved xfail reason look like a real one: `OPEN.md` printed a variable name as one of 7 open rows, NO record linked. Counts were right; nothing counted readability (D-275)
