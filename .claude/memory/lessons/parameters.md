...
- [A saturating knob cannot be the repair](../feedback-a-saturating-knob-cannot-be-the-repair.md) — "extent insensitive, timing misses" read as a repair still available; swept to 1000x the timing SATURATES. `rate ∝ X` can't outrun X. Unreachable, not deferred — but score the OTHER observable in the source's own frame before blaming the form (D-249, narrowed at D-251)
- [A reachable target can be unaffordable](../feedback-a-reachable-target-can-be-unaffordable.md) — the knob crossed the paper's landmark INSIDE its own band for three re-pins; the price was cells at 0.139 g N/g DW against a sourced 0.114. Price a calibration in a conserved budget (D-251)
- [Price a transient against a ceiling of the same kind](../feedback-price-a-transient-against-a-ceiling-of-the-same-kind.md) — [0.08, 0.14] is an END-STATE elemental composition; the source's own whole-run value is 0.0786, under its low edge, so it caps no transient store. Match quantity, basis and moment (D-252)
