...
The suite is ~1250 independent ``solve_ivp`` integrations and saturates every core under ``-n``;
at normal priority that starves whatever else the machine is doing — including the *other* agent
sessions that routinely run their own suites on this box at the same time. Child processes inherit
the priority class on both platforms, so setting it once here covers the whole xdist worker pool.
Run::

    uv run python tools/nicepytest.py -n 12          # full suite, yields to the desktop
    uv run python tools/nicepytest.py tests/test_ensemble.py -q
...
yourself; see ``docs/ARCHITECTURE.md`` and the CLAUDE.md commands block for the measured tradeoff
between ``-n auto`` (all 16 logical CPUs) and a smaller pool that leaves headroom.
...
below. This mirrors ``physical synthesis/scripts/nicepytest.py``, where the trap was first found.
"""
